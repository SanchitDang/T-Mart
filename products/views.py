...
from recommender.recommendations import get_recommendations
from recommender.utils  import add_view_history
...
def home(request):
    ...
    # Get 14 random products
    products = product.objects.order_by('?')[:12]
    rec_products = get_recommendations(user_id=user_id)

    context = {
        'prod': products,
        'rec_prod': rec_products,
        'user_id': user_id
    }
...
def product_detail(request, slug):
    ...
    if request.user.id:
        add_view_history(request.user.id, details.id)
...
def addtocart(request, size, slug):
    ...
    tshirt = product.objects.get(slug=slug)
...
def cart(request):
    ...
    for c in cart:
        ...
        tshirt = product.objects.get(id=tshirt_id)
...
def profile(request):
    ...
    return render(request, 'profile.html', {'users':users, 'user_id':request.user.id})
